...
public class OrderServiceImpl implements OrderService {
	...
	public void add(TbOrder order) {
		...
		for (Cart cart : cartList) {
			...
			tbOrder.setUpdateTime(new Date());//设置订单更新时间
			tbOrder.setUserId(order.getUserId());//设置下单用户
			...
			tbOrder.setSellerId(cart.getSellerId());//设置订单所属商家（从购物车数据中获取）
			
			double money=0;
			//循环购物车明细添加到订单明细
			for (TbOrderItem orderItem: cart.getOrderItemList()) {
				
				orderItem.setId(idWorker.nextId());//设置订单明细ID（使用雪花算法生成）
			...
			tbOrder.setPayment(new BigDecimal(money));//设置总金额
		...
		//如果支付方式是微信支付，插入支付日志，并存入缓存
		if ("1".equals(order.getPaymentType())) {
			TbPayLog payLog = new TbPayLog();
			payLog.setOutTradeNo(idWorker.nextId()+"");//支付订单号
			payLog.setCreateTime(new Date());
			payLog.setTotalFee((long)(total_money*100));//订单总金额
